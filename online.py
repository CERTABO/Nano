...
class LoginCredentials:
    def __init__(self, root):
        ...
        lichess_frame = tk.LabelFrame(
            self.win,
            text="Paste your API Token below",
            labelanchor="n",
            padx=10,
            pady=10,
        )
        lichess_frame.grid(row=0, columnspan=2, padx=10, pady=10, s="EW")
        self.token_lichess_entry = tk.Entry(
            lichess_frame, textvariable=self.token_lichess, width=entry_width
        )
        self.token_lichess_entry.grid(row=0, column=0)

        tk.Button(self.win, text="Save & Exit", command=self.save).grid(
        ...
        tk.Button(
            ...
            command=lambda: webbrowser.open(
                "http://lichess.org/account/oauth/token/create?description=Certabo&scopes[]=board:play",
                new=2,
                autoraise=True,
            ),
...
class GUI:
    def __init__(self):
        ...
        option_frame = tk.LabelFrame(
            root, text="Options", labelanchor="n", padx=10, pady=10
        )
        option_frame.grid(row=0, padx=10, pady=10, s="EW")
        tk.Button(option_frame, text="Play Offline", command=self.play_offline).grid(
            row=0, column=0, s="W"
        )

        launch_frame = tk.LabelFrame(
            root, text="Lichess.org", labelanchor="n", padx=10, pady=10
        )
        launch_frame.grid(row=1, padx=10, pady=10, s="EW")
        tk.Button(launch_frame, text="Launch Website", command=self.launch).grid(
            row=0, column=1, s="W", padx=5
        )
        tk.Button(
            launch_frame, text="Register token", command=login_credentials.open
        ).grid(row=0, column=0, s="W", padx=5)

        status_frame = tk.LabelFrame(
            root, text="Status", labelanchor="n", padx=10, pady=10
        )
    ...
    def play_offline(self):
        ...
        log.info("Switching to Offline Application")
        ...
        if getattr(sys, "frozen", False):
            ...
            os.execl(sys.executable, sys.executable, "main.py", *args)
    ...
    def launch(self):
        ...
        if not token:
            messagebox.showerror(
                "Token missing",
                "Please register your Lichess API token before launching",
            )
            return

        if self.busy:
            messagebox.showwarning("Warning", "Online module is already running")
            return
        self.busy = True

        webbrowser.open("https://lichess.org/", new=2, autoraise=True)
        self.lichess_api_thread = threading.Thread(
            target=lichess.main,
            args=(self.port, token, self.connection_method, self.kill_thread_event),
            daemon=True,
        )
